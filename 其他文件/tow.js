...
function Modify(cuuent) {
    ...
    // 获取当前任务的名称、任务详、任务状态
    let taskName = $(cuuent).parent().parent().find("td:first").text();
    let taskInfo = $(cuuent).parent().parent().find("td:eq(1)").text();
    let taskStatus = $(cuuent).parent().parent().find("td:eq(2)").text();

    // 获取当前选中用户名称
    let selectUsName = $("#exampleFormControlSelect2").val();
        ...
        success: function (result) {
            ...
            if ("operation failed" == result) {
                ...
                for (let i = 0; i < result.length; i++) {
                    ...
                    strHt = strHt + "<option value='" + result[i].usId + "'>" + result[i].usName + "<option>"
                }
